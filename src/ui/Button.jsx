import { Link } from 'react-router-dom';

function Button({ children, disabled, to = '', type, onClick }) {
  const base =
    'inline-block rounded-full bg-yellow-400 text-sm  font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:bg-yellow-300 focus:bg-yellow-300 focus:outline-none focus:ring focus:ring-yellow-300 focus:ring-offset-2 disabled:cursor-not-allowed';

  const styles = {
    primary: base + ' px-4 py-3 sm:px-6 sm:py-4',
    small: base + ' py-2 px-2 md:px-6 md:py-2.5 text-xs',
    round: base + '  py-1 px-2.5 md:px-3.5 md:py-2 text-sm',
    secondary:
      'inline-block text-sm rounded-full bg-transparent border-2 border-stone-400 font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:bg-stone-300 focus:bg-stone-300 focus:outline-none focus:ring focus:ring-sotne-200 focus:ring-offset-2 disabled:cursor-not-allowed  px-4 py-2.5 sm:px-6 sm:py-3.5 hover:text-stone-800',
  };

  if (to.length > 0)
    return (
      <Link className={styles[type]} to={to}>
        {children}
      </Link>
    );

  if (onClick)
    return (
      <button className={styles[type]} disabled={disabled} onClick={onClick}>
        {children}
      </button>
    );

  return (
    <button className={styles[type]} disabled={disabled}>
      {children}
    </button>
  );
}

export default Button;
